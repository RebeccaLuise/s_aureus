...
```{r}
ps_aureus <- readRDS("~/Documents/Forschung/S_aureus_letter/r_analysis_s_aureus/s_aureus/Scripts/S.aureus_only_phyloseq.rds")

ps <- readRDS("~/Documents/Forschung/S_aureus_letter/r_analysis_s_aureus/s_aureus/mOTU_catalogue_totalReadcount.rds")
...
write_csv(prev_df, "rarefied_1q_seq_depth_prevalence.csv")
...
ggsave("seq_depth_normalization_comparison.pdf", height = 6, width = 8)
...
raref_depth <- 6143 # median sequencing depth distribution
n_iter <- 100         # number of rarefaction replicates
...
summary(as_factor(sample_data(rarefied_list[[1]])$timepoint_category))
summary(as_factor(sample_data(rarefied_list[[1]])$study_id))
table(
  sample_data(rarefied_list[[1]])$timepoint_category,
