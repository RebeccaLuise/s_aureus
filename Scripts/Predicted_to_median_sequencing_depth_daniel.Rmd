...
adj_count_1stQ = (S_aureus_count / Total_read_count) * D_star_1stQ,
      present_min   = as.integer(adj_count_1stQ >= 1),
  )
```

```{r}
# correlation before, sig
df_std %>% 
...
df_std %>% 
  ggplot(aes(x=adj_count, y= Total_read_count)) + 
  geom_point() + 
  theme(aspect.ratio = 1) + 
  scale_x_continuous(trans=pseudo_log_trans()) + 
  scale_y_continuous(trans=pseudo_log_trans()) +
...
df_std%>% 
  ggplot(aes(x=adj_count, y= adj_rel)) + 
  geom_point() + 
  theme(aspect.ratio = 1) + 
  #scale_x_continuous(trans=pseudo_log_trans()) + 
  #scale_y_continuous(trans=pseudo_log_trans()) +
...
stats_after  <- get_stats(df_std$Total_read_count, df_std$adj_count)
...
p_after  <- plot_scatter(df_std, Total_read_count, adj_count,
...
```{r}
df_std %>% 
  ggplot(aes(x=S_aureus_count, y= adj_count)) + 
  geom_point() + 
  theme(aspect.ratio = 1) + 
  scale_x_log10() + 
  scale_y_log10() + 
...
# after adjustment
df_std %>%
  mutate(detected = as.integer(adj_count >= 1)) %>%
  group_by(study) %>%
  summarize(median_reads = median(Total_read_count, na.rm=TRUE),
            mean_detect_rate = mean(detected, na.rm=TRUE)) -> summary_by_study_adj

ggplot(df_std, aes(x = Total_read_count, y = as.integer(adj_count >= 1), color = study)) +
  geom_jitter(height = 0.02, alpha = 0.4) +
  stat_smooth(method = "glm", method.args = list(family = "binomial"),
              se = FALSE, aes(group = study)) +
  scale_x_log10() +
...
ggplot(df_std, aes(x = Total_read_count, y = as.integer(adj_count >= 1))) +
  geom_jitter(height = 0.02, alpha = 0.4) +
  stat_smooth(method = "glm", method.args = list(family = "binomial"),
              se = FALSE) +
  scale_x_log10() +
...
```{r}
# Fit logistic regression
fit <- glm(I(adj_count >= 1) ~ log10(Total_read_count), 
