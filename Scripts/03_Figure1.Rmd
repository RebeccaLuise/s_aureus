...
krinko_o <- read_delim("~/Documents/Forschung/S_aureus_letter/r_analysis_s_aureus/s_aureus/Data/clean_mdro_mainz_data_20250114.csv")

primal_md <-  read.delim("~/Documents/Forschung/S_aureus_letter/r_analysis_s_aureus/s_aureus/Data/primal_metadata.csv", sep = ",")
```
...
aureus_df <- aureus_df  %>% 
  left_join(n_df, by=c("study_id", "DOL_category")) %>% 
  mutate(study_short = factor(study_short, levels = study_order),
         x_label = paste0(study_short, ", N=", n_samples),
    x_key   = paste(DOL_category, x_label, sep = "|"))

levels_abund <- aureus_df %>%
...
# Calculate medians per facet (DOL_category)
hline_data_relab <- aureus_df%>%
  group_by(DOL_category) %>%
  summarise(hline_y = round(mean(rel_ab*100, na.rm = TRUE), digits = 5))
hline_data_relab$DOL_category <- factor(hline_data_relab$DOL_category, 
                                              levels = c("DOL 1-8", "DOL 9-42", "DOL 43-455"))

aureus_df$DOL_category<- factor(aureus_df$DOL_category, levels=c("DOL 1-8", "DOL 9-42", "DOL 43-455"))

p5 <- ggplot(aureus_df, aes(x = x_key, y = rel_ab* 100, fill = study_short)) +
...
aureus_df%>% 
  filter(age_days<=42) %>% 
  ggplot(aes(x=age_days, y=(rel_ab*100), color=study_short))+
...
plot_df <- primal_sa_df %>%
  group_by(DOL_category, s_aureus_pos) %>%
...
primal_sa_df$DOL_category <- factor(primal_sa_df$DOL_category , levels=c("DOL 1-8", "DOL 9-42", "DOL 43-455"))
...
primal_sa_df %>% 
  filter(!is.na(DOL_category), DOL_category!="DOL >456") %>% 
  ggplot(aes(y = rel_ab*100, x = DOL_category)) +
...
p4 <- readRDS("~/Documents/Forschung/S_aureus_letter/r_analysis_s_aureus/s_aureus/Data/depth_standardized_S.aureus_prevalence_20251211.rds")
...
ggsave("/Users/rebecca/Documents/Forschung/S_aureus_letter/fig_1_20251211.pdf", fig,  width = 12, height = 8)
```
