...
```{r}
ps <- readRDS( "/Users/rebecca/Documents/Forschung/S_aureus_letter/r_analysis_s_aureus/s_aureus/Data/vfdb_phyloseq.rds")
```
...
```{r}
s_aureus_adj <- read_delim("/Users/rebecca/Documents/Forschung/S_aureus_letter/r_analysis_s_aureus/s_aureus/Data/s_aureus_adjusted_sequencing_depth.csv")
...
#ggsave("virulence_gene_prevalence_depth_stand_s_aureus_pos.#pdf", width = 6, height = 4)
```
...
anno_df <- data.frame(
  Timepoint = c("DOL 9-42",   "DOL 43-455"),
  xmin = c("DOL 9-42|PRIMAL, N=151", "DOL 43-455|PRIMAL, N=7"),
  xmax = c("DOL 9-42|Rahman, N=30", "DOL 43-455|Rahman, N=2"),
  annotations = c("**", "***"),
  y_position = c(2, 2)
)
...
#ggsave("virulence_richness_boxplot.#pdf", height = 4, width = 6)
```
...
df_sumAb %>% 
  ...
  theme(strip.background = element_rect(fill = "#F0F0F0", colour = "black"),
        ...
        panel.spacing = unit(1, "lines"), legend.position = "none") #+
  #guides(fill = guide_legend(nrow = 1, position = "bottom"))
#ggsave("summed_relativeAbundance_VG.#pdf", height = 4, width = 6)
```
...
df_sumAb <- df_Ab_aureus %>% 
group_by(Sample, study_short, Timepoint) %>%
