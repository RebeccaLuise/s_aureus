...
```{r}
psmelt_genus <- readRDS("/Users/rebecca/Documents/Forschung/S_aureus_letter/r_analysis_s_aureus/s_aureus/Data/S.aureus_only_dataframe.rds")
```

```{r}
df <- psmelt_genus %>% 
  select(Sample, Abundance, total_reads, reads, study_id, timepoint_category, subject_id, sample_alias)
...
#saveRDS(p1, "depth_standardized_S.aureus_prevalence.rds")
```
# compare prevalences across seq Depth to rarefied
```{r}
# compare prevalences across seq Depth to rarefied

rare <- read_delim("/Users/rebecca/Documents/Forschung/S_aureus_letter/r_analysis_s_aureus/s_aureus/Data/rarefied_1q_seq_depth_prevalence.csv")
...
df_r_d <- data.frame(
  Timepoint = c("DOL 1-8", "DOL 9-42", "DOL 43-455",
                "DOL 1-8", "DOL 9-42", "DOL 43-455"),
  stan_Depth_prev = c(9.485816, 18.832237, 12.982273, 17.14539, 23.02632, 14.14286),
  
  rar_prev = c(11.82796,21.875,12.5,16.66667,16.94915,17.02128),
...
df_r_d%>%
  ...
  ggplot(aes(x = rar_prev, 
                    y = stan_Depth_prev)) +
  geom_point(aes(shape = Depth, color = Timepoint), size = 3) +
