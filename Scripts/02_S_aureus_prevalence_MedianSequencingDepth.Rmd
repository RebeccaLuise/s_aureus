...
author: "Daniel Podlesny & Rebecca L. Knoll"
date: "`r Sys.Date()`"
output: html_document
---

This script calculates and visualizes *S.aureus* sequencing depth to median sequencing depth and compares different sequencing depth adjustments to the alternative of rarefied S.aureus counts resulting from the rarefied_sequencingDepth.Rmd script
...
```{r}
psmelt_genus <- readRDS("/Users/rebecca/Documents/Forschung/S_aureus_letter/r_analysis_s_aureus/s_aureus/Data/S.aureus_only_dataframe_20251211.rds")
```

```{r}
df <- psmelt_genus %>% 
  select(Sample, Abundance, total_reads, reads, study_id, DOL_category, subject_id, sample_alias)
...
write_csv(df_std, "s_aureus_adjusted_sequencing_depth_20251211.csv")
```
...
# plot sequencing depths per DOL_category and study
```{r}
...
df_for_plot %>% 
  ...
  scale_x_discrete(labels = function(x) sub("^[^|]*\\|", "", x)) +
  labs(
    title = "Sequencing depth per study and DOL category",
...
ggsave("total_seq_depth_20251211.pdf", width = 8, height = 5)
```

# plot tax sequencing depths per DOL_category and study
```{r}
...
df_for_plot %>% 
  ...
  scale_x_discrete(labels = function(x) sub("^[^|]*\\|", "", x)) +
  labs(
    title = "Taxonomic reads per study and DOL_category category",
...
ggsave("tax_seq_depth_20251211.pdf", width = 8, height = 5)
```
...
ggsave("ratio_seq_depth_20251211.pdf", width = 8, height = 5)
```
...
df_std %>% 
  select(present_median, present_actual, Total_read_count) %>% 
  rename(
    "Adjusted to Median Sequencing Depth" = present_median,
    "Prior Depth Adjustment" = present_actual
  ) %>% 
  pivot_longer(
    cols = c("Adjusted to Median Sequencing Depth", "Prior Depth Adjustment"),
    names_to = "method", 
    values_to = "Presence"
  ) %>% 
  mutate(
    ...
    method = factor(method, levels = c(
      ...
      "Adjusted to Median Sequencing Depth"
    )) # <-- order defined here
  ) %>% 
...
#ggsave("depth_adjustement_effect_saureus_detection.pdf", height = 4, width = 7)
...
hline_data_prev <- prev_summary%>%
  group_by(DOL_category) %>%
  summarise(medSeqDepth_prev = median(median, na.rm = TRUE)*100, q1Depth_prev = median(prev_1stQ, na.rm = TRUE)*100, q3Depth_prev = median(prev_3rdQ, na.rm = TRUE)*100, .groups = "drop")

p1 <- ggplot(prev_summary, aes(x = x_key, fill = study_short)) +
  # bar at median
  geom_col(aes(y = median*100), alpha = 0.7, width = 0.6) +
  # error bar from min to max
  geom_errorbar(aes(ymin = q1*100, ymax =q3*100), width = 0.2, colour = "black") +
  #scale_y_continuous(labels = scales::percent_format()) +
  scale_fill_manual(values = fill_palette, breaks = legend_order, drop = FALSE) +
  facet_grid(~ DOL_category, scales = "free_x", space = "free") +
  scale_x_discrete(labels = function(x) sub("^[^|]*\\|", "", x)) +
  labs(
    ...
    y = "Depth-adj. *S. aureus* prevalence (%)", x = "", fill = "Study"
  ) +
  geom_hline(data = hline_data_prev, aes(yintercept = medSeqDepth_prev),
             colour = "black", linetype = 2, inherit.aes = FALSE) +
  theme_minimal(base_size = 14) +
  theme(strip.background = element_rect(fill = "#F0F0F0", colour = "black"),
        strip.text = element_text(face = "bold", size = 14),
 ...
 # guides(fill = guide_legend(nrow = 1, position = ""))
p1
ggsave("prevalence_depth_standardized_s_aureus_20251211.pdf", height = 5, width = 8)
saveRDS(p1, "depth_standardized_S.aureus_prevalence_20251211.rds")
```
# compare prevalences across seq Depth to rarefied
```{r}
# compare prevalences across seq Depth to rarefied

rare <- read_delim("/Users/rebecca/Documents/Forschung/S_aureus_letter/r_analysis_s_aureus/s_aureus/Data/rarefied_1q_seq_depth_prevalence_20251211.csv")
...
df_std %>%
  group_by(DOL_category) %>%
  summarise(
    prev_actual = mean(present_actual, na.rm=TRUE)*100,
    prev_min    = mean(present_min, na.rm = TRUE)*100,
    prev_1stQ   = mean(present_1stQ, na.rm = TRUE)*100,
    prev_median = mean(present_median, na.rm = TRUE)*100,
    prev_3rdQ   = mean(present_3rdQ, na.rm = TRUE)*100,
    prev_max    = mean(present_max, na.rm = TRUE)*100,
    .groups = "drop"
  )

df_r_d <- data.frame(
  DOL_category = c("DOL 1-8", "DOL 9-42", "DOL 43-455",
                "DOL 1-8", "DOL 9-42", "DOL 43-455"),
  stan_Depth_prev = c(6.8, 20.2, 18.04, 9.8, 26.6, 21.4),
  
  rar_prev = c(8.6,21.47,14.28,NA,NA,NA),
  Depth = c("q1","q1","q1", "median","median","median")
)

df_r_d%>%
  filter(Depth=="q1") %>% 
  ggplot(aes(x = rar_prev, 
                    y = stan_Depth_prev)) +
  geom_point(aes(shape = Depth, color = DOL_category), size = 3) +
  geom_smooth(method = "lm", se=FALSE, color = "black") +
  labs(y = "Standardized sequencing depth",
       x = "Rarefied sequencing depth",
       title = "Correlation of sequencing depth estimates for S.aureus prevalence") +
  theme_minimal()+
  stat_cor()#+
  facet_wrap(~Depth)
...
ggsave("prevalence_nonadjusted_aureus_20251211.pdf", height = 5, width = 8)
```
