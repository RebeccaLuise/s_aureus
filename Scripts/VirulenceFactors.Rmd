...
ggplot(gene_counts, aes(x = reorder(GENE, -count), y = count, fill = Cohort)) +
  ...
  theme_minimal()
```
# normalize gene count to Million reads
```{r}
df_new <- df_long %>%
  mutate(reads_per_million = `Gene count` / (reads / 1e6),
         adjusted_presence = ifelse(reads_per_million >= 1, 1, 0))


df_new %>% 
  ggplot(aes(x=Cohort, y=reads_per_million, fill=Cohort))+
  geom_boxplot()+
  scale_y_log10()+
  theme_minimal()+
...
df_new %>%
  group_by(Cohort) %>%
  summarise(mean_prev = mean(adjusted_presence, na.rm = TRUE),
            se = sd(adjusted_presence, na.rm = TRUE) / sqrt(n())) %>%
  ggplot(aes(x = Cohort, y = mean_prev, fill = Cohort)) +
...
df_new%>% 
  ggplot(aes(x = GENE, y = adjusted_presence, fill = Cohort)) +
  geom_col(position = "dodge") +
